...
7. 所有构建只接受严格稳定 SemVer，不提供允许预发布服务端的例外开关。
...
落盘。活动记录同时保存阻止原因；若哨兵未可靠写入，则必须在提交活动记录前销毁并复核 Android
Keystore 凭据主密钥不存在，使 journal 可能引用的全部旧密文跨重启不可解密。密钥销毁失败时不提交
可能刷盘受污染活动指针的记录。哨兵缺失以外的读取异常或损坏一律 Fail Closed。只有完整远端重验证、
候选凭据写入和活动指针提交均成功后，才能清除哨兵。
...
一次来电可能先触发 `CallScreeningService`，随后由系统拨号器通过不同 Contacts URI 多次查询
`Directory Provider`。这些入口共享单例 `SpamNumberRepository`，联网边界必须对相同查询进行复用，避免同一
号码在短时间内重复请求官方或自建 Backend。

复用层采用 1 分钟进程内缓存和 single-flight：号码规范化会先移除空格、连字符与括号，再处理 `+86`、
系统 Contacts URI 生成的 `+` 加 11 位大陆手机号、
明确的 `86 + 11 位大陆手机号` 和移动
多号前缀；缓存键与网络请求使用同一个 canonical number。相同号码、Backend ID、activationId 及网络超时设置
只允许一个进行中请求。成功响应和稳定失败结果均短期复用，缓存最多保留 32 条；Backend 切换、重新激活
或超时设置变化会自然生成新键。source 设置变化后，同一号码最多在剩余缓存期内复用旧联网响应；缓存只保存原始联网结果，黑白名单、标签与
归属地规则仍在每个调用入口重新计算。手动 `forceNetworkQuery` 和独立 `queryNetwork()` 绕过缓存。

缓存不落盘、不输出完整号码，也不改变自建失败时禁止回退官方实时查询的约束。
缓存键与请求使用同一个 Snapshot lease；切换窗口无法把新 Backend 响应写入旧 activation 缓存。完成缓存和
进行中请求各限制为 32 条，达到进行中上限后的请求保留在调用方生命周期内直接执行，不再创建脱离调用方的任务。
Blocked 状态读取到空 Snapshot 时立即 Fail Open，不允许异步任务在稍后动态读取新 Backend。
