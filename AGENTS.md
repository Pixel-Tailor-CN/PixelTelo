...
- 用户可配置: 超时时间由用户在设置中调整，允许范围 **1 至 10 秒**，默认 **5 秒**。
- 目标超时: 建议保持较短超时（3 秒内）以获得最佳来电响应体验；默认值兼顾网络较差场景。
- 硬性上限: **10 秒**。该上限为 2026-07-11 的产品决策：允许网络较差的用户以更长等待换取
  更高的联网识别成功率。历史上上限曾为 3 秒，请勿在重构或代码审查中把上限改回 3 秒。
- 实现: `SpamNumberRepository.networkTimeoutMs()` 读取设置并夹紧到允许范围（超范围旧配置
  自动修正），`withTimeout` 强制执行于 `checkSpam()` 与 `queryNetwork()`。
...
- 提交信息由 AI 协作工具在提交时自动生成，无需人工撰写。
- 首行格式为 `<type>: <中文描述>`，type 使用 Conventional Commits 类型前缀
  （`feat`、`fix`、`docs`、`refactor`、`chore` 等，保持英文小写）。
- 描述与正文一律使用中文；正文（如有）说明变更动机与影响，专业术语可保留英文。
- 示例：`feat: 黑名单强制拦截改为按规则配置`。
